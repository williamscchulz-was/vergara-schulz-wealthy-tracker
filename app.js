...
return `<div class="cat-row"> <div class="name"><span class="dot" style="background:${cat.color}"></span>${cat.icon} ${cat.label}</div> <div class="bar-wrap"><i style="width:${pct}%;background:${cat.color}"></i></div> <div class="v"><b>${pct.toFixed(0)}%</b>${fmtBRL0(val)}</div> </div>`;
}).join(’’);
}
...
return `<div style="display:flex;justify-content:space-between;align-items:center;padding:14px 0;border-bottom:1px solid var(--line);gap:12px"> <div style="flex:1;min-width:0"> <div style="font-size:13px;font-weight:500;color:var(--ink-2);margin-bottom:3px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${cat.icon} ${e.description || '—'}</div> <div style="font-size:11px;color:var(--muted)">${formatDateBR(e.date)} · ${cat.label}</div> </div> <div style="font-family:'Geist Mono',monospace;font-size:13px;font-weight:600;color:var(--ink-2);white-space:nowrap">${fmtBRL(+e.value||0)}</div> </div>`;
}).join(’’);
// Remove last border
...
const cat = CATEGORIES[e.category] || CATEGORIES.outros;
return `<tr data-id="${e.id}"> <td class="mono">${formatDateBR(e.date)}</td> <td>${e.description || '—'}</td> <td><span class="cat-pill" style="background:${cat.color}22;color:${cat.color}"><span class="dot" style="background:${cat.color}"></span>${cat.icon} ${cat.label}</span></td> <td class="mono" style="font-weight:600">${fmtBRL(+e.value||0)}</td> <td></td> </tr>`;
}).join(’’);
...
// Amt subtitle: show total return (valorização + dividendos) if synced
const subEl = $(‘i10EquitySub’);
if (subEl) {
if (state.i10.source === ‘investidor10-sync’ && state.i10.applied > 0) {
// Dividendos: YTD do I10 + histórico dos anos anteriores do Firestore
...
// Retorno total = (patrimônio atual - aplicado + dividendos recebidos) / aplicado
const totalReturn = ((state.i10.equity - state.i10.applied + totalDivs) / state.i10.applied) * 100;
const sign = totalReturn >= 0 ? ‘+’ : ‘’;
const cls = totalReturn >= 0 ? ‘pos’ : ‘neg’;
subEl.innerHTML = `Aplicado ${fmtBRL0(state.i10.applied)} · <span class="${cls}">${sign}${totalReturn.toFixed(2)}%</span> <span style="opacity:.7;font-size:11px">(com dividendos)</span>`;
} else {
...
svg += `<defs> <linearGradient id="barGradient" x1="0" y1="0" x2="0" y2="1"> <stop offset="0%" stop-color="${color}" stop-opacity="1"/> <stop offset="100%" stop-color="${color}" stop-opacity=".5"/> </linearGradient> <linearGradient id="barGradientCurrent" x1="0" y1="0" x2="0" y2="1"> <stop offset="0%" stop-color="${color}" stop-opacity="1"/> <stop offset="100%" stop-color="${color}" stop-opacity=".7"/> </linearGradient> </defs>`;
...
svg += `<g class="tooltip-group"> <rect class="tooltip-bg" x="${tipX - textW/2}" y="${tipY - 26}" width="${textW}" height="32" rx="6"/> <text class="tooltip-text" x="${tipX}" y="${tipY - 13}" text-anchor="middle">${labelText}</text> <text class="tooltip-value" x="${tipX}" y="${tipY + 1}" text-anchor="middle">${valueText}</text> </g>`;
}
...
if (sorted.length === 0) {
tbody.innerHTML = `<tr><td colspan="5"><div class="empty-table"><h4>Sem dados anuais</h4><p>Clique em "Adicionar ano" para começar.</p></div></td></tr>`;
return;
}
...
return `<div class="asset-row-slim"> <div class="asset-ticker-slim">${a.ticker || '—'}</div> <div class="asset-value-slim">${fmtBRL0(+a.equity || 0)}</div> <div class="asset-appr-slim ${apprCls}">${apprSign}${appr.toFixed(1)}%</div> </div>`;
}).join(’’);
}
