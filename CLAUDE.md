...
- **Dívida técnica (pendente — William vai olhar com calma):** os GitHub Actions usam **Node 20**, deprecado pelo GitHub (forçado a Node 24 em 16/06/2026, removido em 16/09/2026 — aparece como aviso amarelo nos deploys). Atualizar antes de set/2026: `actions/checkout@v4`→`@v5` e trocar `FirebaseExtended/action-hosting-deploy@v0` por deploy via firebase CLI (`npm i -g firebase-tools && firebase deploy`) usando o secret `FIREBASE_SERVICE_ACCOUNT`.
