...
- **Patrimônio histórico blindado**: `HISTORICAL_EQUITY` (mapa `{ano: equity}` em `public/js/app.js`) guarda o patrimônio de fim de ano 2020-2025 — dado real do William que o I10 não devolve confiável pra carteira nova. `yearEquity(y)` usa o Firestore se houver (>0) senão o fallback embutido. Usado por `renderPLChart` e `renderYearlyTable`, então o gráfico de patrimônio por ano nunca mais some por wipe de sync. Edição manual via "+ Year" vence o fallback. Pra ajustar/estender anos: editar o mapa.
